...
public class TaskTest {
    ...
    @Test
    public void WaitForResultReturnsResponse() {
        String dummyRequestId = "dummyRequestId";
        String dummyResponseBody = "dummyResponseBody";

        ReceivedEventInterface dummyEvent = new ReceivedEventInterface();
        dummyEvent.requestId = dummyRequestId;
        dummyEvent.responseBody = dummyResponseBody;

        Util.pushResponseToBuffers(dummyEvent);

        String response = (String) Task.waitForResult(dummyEvent.requestId);

        assertEquals(dummyResponseBody,response);

        Util.clearResponseFromBuffers(dummyEvent);
    }

    @Test
    public void TaskDeterminerReturnsValidTask() {
        TASK_TYPE task = Task.determineTask(stringsAddRequestBody);
        SUB_TASK_TYPE subtask = Task.determineSubTask(task, stringsAddRequestBody);

        Mockito.when(containerInfo.fetchContainerInfo()).thenReturn(new ContainerInfoInterface(dummyContainerInfo.id, dummyContainerInfo.service));
        Mockito.when(containerInfo.fetchEventContainer()).thenReturn(new ContainerInfoInterface(dummyEventContainerInfo.id, dummyEventContainerInfo.service));

        TaskInterface determinedTask = Task.taskDeterminer(task, subtask, stringsAddRequestBody, containerInfo);

        assertEquals(dummyContainerInfo.id ,determinedTask.containerId);
        assertEquals(dummyContainerInfo.service ,determinedTask.service);
        assertEquals(dummyEventContainerInfo.id ,determinedTask.serviceContainerId);
        assertEquals(dummyEventContainerInfo.service ,determinedTask.serviceContainerService);
    }
